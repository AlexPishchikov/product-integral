...
pub fn parse_input_params(args : Vec<String>) -> (FunctionParams, PlotType) {
    ...
    let input_max = args[7].parse::<f64>();
    ...
    plot_type.integral   = if input_plot_type.chars().nth(3) == Some('1') {true} else {false};

    match input_f {
        Ok(input_f)  => function_params.f = input_f,
        Err(input_f) => {
            eprintln!("{:?}", input_f);
            process::exit(1);
        },
    ...
    match input_max {
        Ok(input_max)  => function_params.max = input_max,
        Err(input_max) => {
            eprintln!("{:?}", input_max);
            process::exit(1);
        },
